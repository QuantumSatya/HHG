...
# Parameters
num_frames = 100000  # choose number of frames to be analyzed
...
for ii in range(number_of_files):
    ...
    with tiff.TiffFile(file_path) as tif:
        ...
        for i in tqdm(range(total_frames - 1)):  # Loop over frames, ensuring enough frames for `frame2`
            ...
            S = np.abs(np.fft.fftshift(np.fft.fft2(frame1)))
            N = np.abs(np.fft.fftshift(np.fft.fft2(frame2)))
...
# --- Plot 1: RJDP ---
im0 = axs[0].imshow((RJDP), origin='lower')
